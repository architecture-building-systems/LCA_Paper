...
The analysis looks \textcolor{magenta}{\sout{at the embodied carbon in} into the environmental impacts, due to the} production, operation, and disposal of the Adaptive Solar Facade (ASF).

\subsection{\textcolor{magenta}{\sout{Embodied Energy} Life Cycle Inventory} and Assumptions}
...
The inventory data was obtained through technical drawings, research papers describing the technology and \textcolor{magenta}{\sout{personal conversations} expert judgement}. The Ecoinvent v3.1 database is used as the main LCI database. To keep assumptions consistent, only data from this database is used. \textcolor{magenta}{\textit{(Is that so? Didn't we add a CIGS dataset?)}} Furthermore, the cut-off approach is used for \textcolor{magenta}{allocation of} recycling and landfill. This means that recycling does not generate any credit for the product and resulting benefits are not taken into account. Furthermore the use of recycled products do not bear the burden of processes higher up the chain.\textcolor{magenta}{\textit{(We don't do any system expansion?)}}\\

For the impact assessment the ReciPe midpoint (H) \textcolor{magenta}{\sout{allocation method} indicator} is used. \textcolor{magenta}{\textit{(This is a little inconsistent. We should consistently use the term environmental impact as opposed to carbon content, etc.)}} The impact assessment is performed using the OpenLCA impact assessment tool.\\
